<?php include 'header.php';?>	

<!-- breadcrumbs start here -->

<div class="breadcrumbs_section" style="background-image: url(assets/images/breadcrumbs_product_bg.jpg);">
	<div class="container">
		<div class="page_title">
			<h1>Name of the Sub-category</h1>
		</div>
		<div class="breadcrumbs_list">
			<ul>
				<li><a href="index.html">Home </a></li>/ 
				<li>Products</li>
			</ul>
		</div>
	</div>
</div>

<!-- breadcrumbs end here -->



<!-- Page content start here -->

<div class="wrapper_page">
	<div class="container">

			<div class="product-list-top">
				<div class="product-category">
					<p>main category</p>
					<select>
						<option>Sub-Category one</option>
						<option>Sub-Category one</option>
						<option>Sub-Category one</option>
					</select>
				</div>
				<div class="product-search">
					<p>Search</p>
					<div class="search-bar">
						<input type="text" name="">
						<button><i class="fa fa-search" aria-hidden="true"></i></button>
					</div>
				</div>
			</div>

			<div class="products-details-page">
				<div class="products-details-img">
					<img src="assets/images/product-list-1.png"/>
				</div>
				<div class="products-details-info">
					<h1>Product name</h1>
					<h2>Sub- Category Name</h2>
					<p>Information about the product comes here Information about the product comes here Information about the product comes here Information about the product comes here</p>
				</div>
			</div>
		
	</div>


<div class="other-categories">
	<div class="container">
		<p class="other_title">Other categories</p>
		<div class="other-categories-slider">
			<div class="other-categories-row">
				<div class="other-categories-img">
					<img src="assets/images/product-list-1.png"/>
				</div>
				<h3 class="categories-title"><a href="#">Name of the main category</a></h3>
			</div>
			<div class="other-categories-row">
				<div class="other-categories-img">
					<img src="assets/images/product-list-1.png"/>
				</div>
				<h3 class="categories-title"><a href="#">Name of the main category</a></h3>
			</div>
			<div class="other-categories-row">
				<div class="other-categories-img">
					<img src="assets/images/product-list-1.png"/>
				</div>
				<h3 class="categories-title"><a href="#">Name of the main category</a></h3>
			</div>
			<div class="other-categories-row">
				<div class="other-categories-img">
					<img src="assets/images/product-list-1.png"/>
				</div>
				<h3 class="categories-title"><a href="#">Name of the main category</a></h3>
			</div>
			<div class="other-categories-row">
				<div class="other-categories-img">
					<img src="assets/images/product-list-1.png"/>
				</div>
				<h3 class="categories-title"><a href="#">Name of the main category</a></h3>
			</div>
			<div class="other-categories-row">
				<div class="other-categories-img">
					<img src="assets/images/product-list-1.png"/>
				</div>
				<h3 class="categories-title"><a href="#">Name of the main category</a></h3>
			</div>
		</div>
	</div>
</div>

</div>

<!-- Page content end here -->


<?php include 'footer.php';?>
</body>
</html>